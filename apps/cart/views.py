    ...
    if request.user.is_authenticated:
        if cart:
            if request.method == 'POST':
                ...
                # Đánh dấu giỏ hàng đã đặt hàng
                cart.ordered = True
                cart.save(update_fields=['ordered'])

                # Tạo giỏ hàng mới cho khách hàng
                Cart.objects.create(total=0, profile=profile)

                # Xử lý logic liên quan đến seller
                for cp in cartproduct:
                    seller = cp.product.seller  # Lấy seller từ sản phẩm
                    if cp.quantity is None or cp.subtotal is None:
                        messages.error(request, "Cart product data is incomplete.")
                        return redirect("apps.accounts:cart")
                    
                    # Tìm hoặc tạo sale liên quan đến seller
                    sale = Sales.objects.filter(product=cp.product, rate=cp.rate, company=seller).last()
                    if sale is None:
            ...
            messages.warning(request, 'Add items to Cart.')
